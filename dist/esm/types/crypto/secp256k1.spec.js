...
describe('secp256k1', () => {
    ...
    it('secp256k1_accPubkey_cosmos', async () => {
        ...
        const publicKeyCLI = cosmosclient.codec.protoJSONToInstance(pubKeyCLI);
        //publickey from mnemonic
        const mnemonic = 'joy furnace inject spot engine source alpha turn east visa abstract cousin shell express weasel math perfect tiger quality camp mansion desert web jaguar';
    ...
    it('secp256k1_accPubkey_ununifi', async () => {
        ...
        //publickey from mnemonic
        const mnemonic = 'chest flight brain grocery flock elephant gloom gaze diet girl subway again extra spider monitor kiss explain paper beauty ordinary ship dry oxygen shield';
        const privKey = new proto.cosmos.crypto.secp256k1.PrivKey({
            key: await cosmosclient.generatePrivKeyFromMnemonic(mnemonic),
        });
        const pubkey = privKey.pubKey();
